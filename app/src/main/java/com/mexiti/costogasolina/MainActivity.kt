        ...
        setContent {
            CostoGasolinaTheme {
                Surface(
                    ...
                    color = Color.LightGray // Fondo gris clarito
                ) {
                    CostGasLayout()
                }
            }
...
fun CostGasLayout() {
    ...
    var cantLitroEntrada by remember { mutableStateOf("") }
    var propinaEntrada by remember { mutableStateOf("") }

    // Variable que guarda si el switch está prendido (true) o apagado (false)
    ...
    // Si darPropina es true, usamos lo que se escribe. Si es false, la propina es 0.0
    val propina =
        if (darPropina)
                (propinaEntrada.toDoubleOrNull() ?: 0.0)
        else 0.0
    ...
    Column(
        ...
        EditNumberField(
            ...
            onValueChanged = { cantLitroEntrada = it }
        )
        ...
        // Cambia el valor de 'darPropina'
        Text(text = "¿Deseas agregar propina?")
        Switch(
            checked = darPropina,
            onCheckedChange = { darPropina = it }
        )

        Text(text = stringResource(R.string.monto_total, monto))
    }
}
@Composable
fun EditNumberField(
